...
--------------------------------""")
    ...
    def auto_commit(self, optimize_ctx: OptimizeContext) -> None:
        ...
        if not project_key or not isinstance(project_key, str) or not project_key.strip():
            log.error(f"Invalid project_key: {repr(project_key)}")
            raise ValueError(f"project_key must be a non-empty string, got: {type(project_key)}")
        
        if not self._key or not isinstance(self._key, str) or not self._key.strip():
            log.error(f"Invalid agent key: {repr(self._key)}")
            raise ValueError(f"agent key must be a non-empty string, got: {type(self._key)}")

        # Strip whitespace and URL encode the path components to handle any special characters
        project_key = project_key.strip()
        agent_key = self._key.strip()
        
        # URL encode only if necessary (most LaunchDarkly keys are already URL-safe)
        # Use quote with safe='' to encode special characters, but preserve alphanumerics and common safe chars
        encoded_project_key = urllib.parse.quote(project_key, safe='')
        encoded_agent_key = urllib.parse.quote(agent_key, safe='')
        
        base_url = "https://api.launchdarkly.com"
        url_endpoint = f"{base_url}/api/v2/projects/{encoded_project_key}/ai-configs/{encoded_agent_key}/variations"

        # Validate the URL is properly formed
        try:
            parsed_url = urllib.parse.urlparse(url_endpoint)
            if not parsed_url.scheme or not parsed_url.netloc:
                raise ValueError(f"Invalid URL format: {url_endpoint}")
        except Exception as e:
            log.error(f"Failed to parse URL: {url_endpoint}. Error: {e}")
            raise
        
        request_body = {
          "key": f"autogenerated-variation-{uuid.uuid4()}",
          "name": f"Autogenerated Variation {uuid.uuid4()}",
          "modelConfigKey": f"OpenAI.{optimize_ctx.current_model}",
          "model": {
        ...
        # Does not use Bearer
        req.add_header('Authorization', f'{self.api_key}')
        ...
        try:
            ...
            log.error(
                f"Auto-commit failed with URL error: {error_msg}. "
                f"URL: {url_endpoint}"
            )
            raise
        except Exception as e:
